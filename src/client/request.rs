...
impl<S: std::fmt::Display, V: std::fmt::Display> From<BTreeMap<S, V>> for Param {
    fn from(value: BTreeMap<S, V>) -> Self {
        Self::Map(value.into_iter().map(|(k, v)| (k.to_string(), v.to_string())).collect())
    }
}
...
impl Param {
    fn encode(&self, name: &str) -> String {
        match self {
            Self::Array(values) => url::form_urlencoded::byte_serialize(values.join(",").as_bytes()).collect::<String>(),
            Self::Value(value) => url::form_urlencoded::byte_serialize(value.as_bytes()).collect::<String>(),
            Self::Map(values) => {
                values.iter()
                    .map(|(k, v)| {
                        format!("{}={}",
                            url::form_urlencoded::byte_serialize(format!("{name}[{k}]").as_bytes()).collect::<String>(),
...
impl Params {
    ...
    pub fn extend<K: std::fmt::Display, P: Into<Param>>(&mut self, params: impl IntoIterator<Item=(K, P)>) {
        self.0.extend(params.into_iter().map(|(k, v)| (k.to_string(), v.into())));
    }
...
impl std::fmt::Display for Params {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params = self.0
            ...
            .map(|(key, param)| {
                if param.is_array() {
                    format!("{}={}", url::form_urlencoded::byte_serialize(format!("{key}[]").as_bytes()).collect::<String>(), param.encode(key))
...
impl Request {
    ...
    pub fn add_param_opt<P: Into<Param>>(&mut self, key: impl std::fmt::Display, value: Option<P>) -> &mut Self {
    ...
    pub fn param_opt<V: Into<Param>>(mut self, key: impl std::fmt::Display, value: Option<V>) -> Self {
        if let Some(value) = value {
